...
case "$FILE_PATH" in
  */.agents/logs/*|*/.agents/backups/*) exit 0 ;;
esac

BACKUP_DIR="$PROJECT_DIR/.agents/backups/$(date +%Y-%m-%d)"
mkdir -p "$BACKUP_DIR"
...
find "$PROJECT_DIR/.agents/backups" -maxdepth 1 -type d -mtime +7 -exec rm -rf {} \; 2>/dev/null
